...
@app.get("/legal/delete-account", response_class=HTMLResponse)
@app.get("/api/legal/delete-account", response_class=HTMLResponse)
async def delete_account_instructions(lang: str = "pt"):
    """Public page with account deletion instructions (required by Google Play)."""
    if lang == "pt":
        content = """
<h1>Exclusão de Conta</h1>
<p class="meta">BizRealms - Instruções para exclusão de conta e dados</p>
<h2>Como excluir sua conta</h2>
<p>Para solicitar a exclusão da sua conta BizRealms e todos os dados associados:</p>
